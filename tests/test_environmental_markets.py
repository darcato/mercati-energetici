...
class TestMercatiAmbientali:
    async def test_general_condtions(self, mercati_ambientali):
        general_conditions = await mercati_ambientali.general_conditions()
        ...
        general_conditions = await mercati_ambientali.general_conditions(language="en")
        assert general_conditions["lingua"] == "EN"
        general_conditions = await mercati_ambientali.general_conditions(language="IT")
        assert general_conditions["lingua"] == "IT"
        general_conditions = await mercati_ambientali.general_conditions(language="it")
        assert general_conditions["lingua"] == "IT"
        with pytest.raises(MercatiEnergeticiError):
            await mercati_ambientali.general_conditions(language="FR")

    async def test_disclaimer(self, mercati_ambientali):
        disclaimer = await mercati_ambientali.disclaimer()
        ...
        disclaimer = await mercati_ambientali.disclaimer(language="en")
        assert disclaimer["lingua"] == "EN"
        disclaimer = await mercati_ambientali.disclaimer(language="IT")
    ...
    async def test_trading_results(self, mercati_ambientali):
        ...
        with pytest.raises(MercatiEnergeticiRequestError):
            await mercati_ambientali.trading_results("NONEXISTENT")
        with pytest.raises(MercatiEnergeticiRequestError):
            await mercati_ambientali.trading_results("GO", day=date(2023, 3, 24))
