...
class TestMercatiGas:
    async def test_general_condtions(self, mercati_gas):
        general_conditions = await mercati_gas.general_conditions()
        ...
        general_conditions = await mercati_gas.general_conditions(language="en")
        assert general_conditions["lingua"] == "EN"
        general_conditions = await mercati_gas.general_conditions(language="IT")
        assert general_conditions["lingua"] == "IT"
        general_conditions = await mercati_gas.general_conditions(language="it")
        assert general_conditions["lingua"] == "IT"
        with pytest.raises(MercatiEnergeticiError):
            await mercati_gas.general_conditions(language="FR")

    async def test_disclaimer(self, mercati_gas):
        disclaimer = await mercati_gas.disclaimer()
        ...
        disclaimer = await mercati_gas.disclaimer(language="en")
        assert disclaimer["lingua"] == "EN"
        disclaimer = await mercati_gas.disclaimer(language="IT")
        assert disclaimer["lingua"] == "IT"
        disclaimer = await mercati_gas.disclaimer(language="it")
        assert disclaimer["lingua"] == "IT"
        with pytest.raises(MercatiEnergeticiError):
            await mercati_gas.disclaimer(language="FR")

    async def test_markets(self, mercati_gas):
        markets = await mercati_gas.markets()
    ...
    async def test_continuous_trading_results(self, mercati_gas):
        ...
        for market in markets:
            results = await mercati_gas.continuous_trading_results(
                market["prodotto"],
        ...
        with pytest.raises(TypeError):
            await mercati_gas.continuous_trading_results()
        with pytest.raises(MercatiEnergeticiRequestError):
            await mercati_gas.continuous_trading_results("NONEXISTENT")
    ...
    async def test_auction_trading_results(self, mercati_gas):
        ...
        for market in markets:
            results = await mercati_gas.auction_trading_results(
                market["prodotto"],
                day=datetime.strptime(str(market["data"]), "%Y%m%d").date(),
            )
            assert results is not None
            ...
            for result in results:
                ...
                assert result["prodotto"] == market["prodotto"]
        with pytest.raises(TypeError):
            await mercati_gas.auction_trading_results()
        with pytest.raises(MercatiEnergeticiRequestError):
            await mercati_gas.auction_trading_results("NONEXISTENT")
    ...
    async def test_stored_gas_trading_results(self, mercati_gas):
        ...
        for market in markets:
            results = await mercati_gas.stored_gas_trading_results(
                market["prodotto"],
                day=datetime.strptime(str(market["data"]), "%Y%m%d").date(),
            )
            assert results is not None
        ...
        with pytest.raises(TypeError):
            await mercati_gas.stored_gas_trading_results()
        with pytest.raises(MercatiEnergeticiRequestError):
            await mercati_gas.stored_gas_trading_results("NONEXISTENT")
